...
async function updateTheme(req, res) {
  const user = await User.findByIdAndUpdate(req.user._id, { theme: "dark" }, { new: true }).select("-password");
...
async function updateSettings(req, res) {
  const { name, email, currency, notificationsEnabled, autoCategoryEnabled, defaultCategory } = req.body;
  const trimmedName = name ? name.trim() : "";
  const normalizedEmail = email ? email.trim().toLowerCase() : "";
